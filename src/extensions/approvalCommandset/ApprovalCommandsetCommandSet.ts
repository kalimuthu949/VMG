...
var listweb = Web(
  "https://veritasmg.sharepoint.com/sites/VMGsProjectManagementTeam"
);
    ...
    if (event.selectedRows.length > 0) {
      ...
      for (var i = 0; i < event.selectedRows.length; i++) {
        ...
        event.selectedRows[i]["_values"].forEach(async function (val, key) {
          if (key == "ContentType" && val == "Folder") flgDoc = false;
          ...
          if (key == "FileRef") {
            ...
            if (HoldFolderName.toLowerCase() == RootFolderName.toLowerCase()) {
              ...
              ApprovalFolderURL =
                RootFolderUrl.slice(0, -2).join("/") + "/" + ApprovalFolderName;
              RejectedFolderURL =
                RootFolderUrl.slice(0, -2).join("/") + "/" + RejectedFolderName;
        ...
        arrSelectedRows.push({
          ApprovalFolderURL: ApprovalFolderURL,
          RejectedFolderURL: RejectedFolderURL,
          FileName: FileName,
          FileURL: FileURL,
  ...
  for (var i = 0; i < arrSelectedRows.length; i++) {
    if (arrSelectedRows[i].ApprovalFolderURL && arrSelectedRows[i].FileName) {
      ...
      const destinationUrl =
        arrSelectedRows[i].ApprovalFolderURL +
        "/" +
        arrSelectedRows[i].FileName;
      ...
      //! Resume without Jobcode
      const resumeFolderUrl =
        "/sites/HRPrivateNew/Shared%20Documents/1. Recruiting/2. Resumes/" +
        arrSelectedRows[i].FileName;

      // await sp.web.getFileByServerRelativePath(arrSelectedRows[i].FileURL).moveTo(destinationUrl).then(async function(data)
      await sp.web
        .getFileById(arrSelectedRows[i].ItemId)
        .copyTo(resumeFolderUrl, true)
        .then(async () => {
          await sp.web
            .getFileById(arrSelectedRows[i].ItemId)
            ...
            .then(async function (data) {
              ...
              await listweb.lists
                ...
                .then(async (item) => {
                  ...
                  ResumeURL =
                    Url.split("Screened Resumes").join("Approved Resumes");
                })
                .catch((error) => {
                  Dialog.alert(`someting went wrong.please try again`).then(
                    function () {
                      location.reload();
                      return false;
                    }
...
async function updateRejectFolder() {
  ...
  for (var i = 0; i < arrSelectedRows.length; i++) {
    if (arrSelectedRows[i].RejectedFolderURL && arrSelectedRows[i].FileName) {
      // destination is a server-relative url of a new file
      const destinationUrl =
        arrSelectedRows[i].RejectedFolderURL +
        "/" +
        arrSelectedRows[i].FileName;
        ...
        .then(async function (data) {
          ...
          await listweb.lists
            ...
            .then(async (item) => {
              ...
              ResumeURL =
                Url.split("Screened Resumes").join("Rejected Resumes");
            })
            .catch((error) => {
              console.log(error);
            });
